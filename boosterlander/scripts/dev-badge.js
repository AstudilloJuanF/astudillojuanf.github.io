...
function badgeOnScroll(){
	...
	if(window.pageYOffset > (footer.offsetTop - window.innerHeight)){
		...
		badge.style.margin = 'auto auto auto 1.5rem';
		badge.style.bottom = '1.5rem';
	}
}

function hideBadge(){
	badge.style.display = '';
	window.removeEventListener('scroll', badgeOnScroll);
}

function showBadge(){
	badge.style.display = 'block';

	window.addEventListener('scroll', badgeOnScroll);
